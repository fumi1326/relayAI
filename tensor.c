...
int relay_req(char *system_cmd)
{
#define MAXLINE 1024
  char sbuf[MAXLINE], rbuf[MAXLINE];
  int wlen, rlen;

  wlen = strlen(system_cmd);
  if (writen(relay_fd, system_cmd, wlen) != wlen)  { fprintf(stderr, "relay_fd: writen error\n"); exit(1); }
  if ((rlen = readn(relay_fd, rbuf, MAXLINE)) < 0) { fprintf(stderr, "relay_fd: readn error\n");  exit(1); }
  usleep(160000); /* 160msec */
}

void multiply_float2D(int THREAD, int LANE, float2D *C, const float2D *A, int transA, const float2D *B, int transB, int spike_mode, int relay_mode) /* C=AxB, transA,or B */ {
  ...
  int row, col, k, l, d, e, wlen, rlen;

#define uTH(a) ((a)            <0.1?0:1)
#define sTH(b) ((b)<-0.1?-1:(b)<0.1?0:1)
  char buf[1024];

  if (!transA && !transB) { /* A: m �� k ��  B: k �� n ��  C: m �� n �� */
    for (row=0; row<m; row++) {
      for (col=0; col<n; col++) {
	C->data[row*n+col] = 0;
	if (spike_mode) {
#define RELAYBITS 8
	  for (k=0; k<ka; k++) {
	    C->data[row*n+col] += uTH(A->data[row*ka+k]) * sTH(B->data[k*n+col]);
	    if (C->data[row*n+col] < -(1<<(RELAYBITS-1)) || C->data[row*n+col] > (1<<(RELAYBITS-1))-1) {
	      printf(" V(%d)", (int)C->data[row*n+col]);
	      C->data[row*n+col] = 0;
	    }
	  }
	  if (relay_mode) {
	  ...
	  for (k=0; k<ka; k++) C->data[row*n+col] += A->data[row*ka+k] * B->data[k*n+col];
	}
      }
    }
  }
  else if (transA && !transB) { /* A: k �� m ��  B: k �� n ��  C: m �� n �� */
    for (row=0; row<m; row++) {
      for (col=0; col<n; col++) {
	C->data[row*n+col] = 0;
	if (spike_mode) {
	  for (k=0; k<ka; k++) C->data[row*n+col] += uTH(A->data[k*m+row]) * sTH(B->data[k*n+col]);
	}
	else {
	  for (k=0; k<ka; k++) C->data[row*n+col] += A->data[k*m+row] * B->data[k*n+col];
	}
      }
    }
  }
  else if (!transA && transB) { /* A: m �� k ��  B: n �� k ��  C: m �� n �� */
    for (row=0; row<m; row++) {
      for (col=0; col<n; col++) {
	C->data[row*n+col] = 0;
	if (spike_mode) {
	  for (k=0; k<ka; k++) C->data[row*n+col] += uTH(A->data[row*ka+k]) * sTH(B->data[col*ka+k]);
	}
	else {
	  for (k=0; k<ka; k++) C->data[row*n+col] += A->data[row*ka+k] * B->data[col*ka+k];
	}
      }
    }
  }
  /* transA && transB is not used */
  else if (transA && transB) { /* A: k �� m ��  B: n �� k ��  C: m �� n �� */
    for (row=0; row<m; row++) {
      for (col=0; col<n; col++) {
	C->data[row*n+col] = 0;
	if (spike_mode) {
	  for (k=0; k<ka; k++) C->data[row*n+col] += uTH(A->data[k*m+row]) * sTH(B->data[col*ka+k]);
	}
	else {
	  for (k=0; k<ka; k++) C->data[row*n+col] += A->data[k*m+row] * B->data[col*ka+k];
	}
      }
    }
  }
